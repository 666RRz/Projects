...
function makeTodoItemFromData(data) {
  ...
  // Проверяем наличие необходимых полей и преобразуем к нужным типам
  const todoItem = {
    name: data.name ? String(data.name) : '',
    isDone: Boolean(data.isDone),
    repeatedDays: Array.isArray(data.repeatedDays) ? data.repeatedDays : [],
  ...
  if (errors.length) {
    throw new TodoApiError(422, { errors });
  }
...
function createTodoItem(data) {
  ...
  // Генерация id на сервере (если нужно)
  if (!newItem.id) {
    newItem.id = crypto.randomUUID(); // Подставьте свою логику генерации id
  }

  const todoList = getTodoList();
  todoList.push(newItem);
  writeFileSync(DB_FILE, JSON.stringify(todoList, null, 2), { encoding: 'utf8' });
...
function updateTodoItem(itemId, data) {
  ...
  const updatedItem = makeTodoItemFromData({ ...todoItems[itemIndex], ...data });

  // Заменяем старый объект новым
  todoItems[itemIndex] = updatedItem;

  writeFileSync(DB_FILE, JSON.stringify(todoItems, null, 2), { encoding: 'utf8' });
